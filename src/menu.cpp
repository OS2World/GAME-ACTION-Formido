...
void load_menus() {
	...
	if(!screen->format->palette)
		tmp = IMG_LoadJPG_RW(SDL_RWFromFile(DAT("menu.dat"), "rb"));
	else
		tmp = IMG_LoadPNG_RW(SDL_RWFromFile(DAT("menu_8bit.dat"), "rb"));
	if(tmp == NULL)
	...
	if(!screen->format->palette)
		tmp = IMG_LoadJPG_RW(SDL_RWFromFile(DAT("menu2.dat"), "rb"));
	else
		tmp = IMG_LoadPNG_RW(SDL_RWFromFile(DAT("menu2_8bit.dat"), "rb"));
	if(tmp == NULL)
		error_msg("Unable to load menus!\n%s", IMG_GetError());
	...
	if(!screen->format->palette)
		tmp = IMG_LoadJPG_RW(SDL_RWFromFile(DAT("menu3.dat"), "rb"));
	else
		tmp = IMG_LoadPNG_RW(SDL_RWFromFile(DAT("menu3_8bit.dat"), "rb"));
	if(tmp == NULL)
		error_msg("Unable to load menus!\n%s", IMG_GetError());
	...
	// Load the menu arrow
	tmp = IMG_LoadPCX_RW(SDL_RWFromFile(DAT("menu_arrow.dat"), "rb"));
	if(tmp == NULL)
		error_msg("Unable to load menus!\n%s", IMG_GetError());

...
void load_bgrounds() {
	...
	if(orig_bground == NULL) {
		if(!screen->format->palette)
			tmp = IMG_LoadJPG_RW(SDL_RWFromFile(DAT("ground.dat"), "rb"));
		else
			tmp = IMG_LoadPNG_RW(SDL_RWFromFile(DAT("ground_8bit.dat"), "rb"));
		if(tmp == NULL)
	...
	if(col_map == NULL) {
		col_map = IMG_LoadPCX_RW(SDL_RWFromFile(DAT("collisionmap.dat"), "rb"));
		if(col_map == NULL)
...
// Music playing stuff
int music_num = -1;
char *music_files[] = {"data/music1.ogg", "data/music2.ogg", "data/music3.ogg"};
